...
> = {
  egig: {
    ...
    그 과정에서 단순한 기술 습득을 넘어, 팀워크와 커뮤니케이션의 중요성을 깊이 있게 체감할 수 있었습니다.
    
    이러한 활동을 통해 저는 "기술은 단순한 도구가 아니라 사회를 변화시키는 핵심 힘"이라는 사실을 온몸으로 깨달았으며,  
    단지 이론적인 관심을 넘어서 진정으로 프로그래밍 기술과 컴퓨터 시스템에 대한 실질적인 학습 필요성을 절감하게 되었습니다.
  ...
  ssafy: {
    ...
    무엇보다 의미 있었던 점은, 단순한 기술 습득을 넘어 팀원들과 함께 문제를 해결해 나가는 과정이었습니다.  
    학우들과의 프로젝트와 알고리즘 스터디, 페어 프로그래밍을 진행하며 협력, 이해심, 그리고 끈기 있게 문제를 해결하는 힘을 배워갔습니다.  
    ...
    비전공자였던 저를 개발자로 성장시킬 수 있도록 충분한 기반을 마련해준 터전이었으며, 협업과 실천을 통해 저의 가능성을 확인할 수 있었던 값진 시간들이었습니다.
    `,
  },
  react: {
    ...
    React는 단순한 프레임워크를 넘어, 프론트엔드 설계 사고방식을 확장시켜 준 기술이었습니다.  
  ...
  tailwind: {
    ...
    Tailwind CSS는 저에게 프론트엔드 개발의 생산성과 미적 감각을 동시에 끌어올려준 핵심 도구였으며, 지금은 가장 주력으로 사용하는 CSS 프레임워크로 자리잡았습니다.
    `,
  },
  typescript: {
    ...
    또한 타입 정의를 기반으로 한 에디터의 자동완성 기능과 타입 추론, 객체 구조에 대한 명시적 표현 덕분에 개발 경험도 한층 더 향상되었습니다.  
    함수의 매개변수, 반환 타입, API 응답 객체 등에 타입을 지정함으로써 실수를 줄이고 유지보수의 안정성을 높일 수 있었습니다.
    
    TypeScript는 단순한 문법의 확장이 아니라, 더 나은 개발 습관을 만들어주는 도구라고 느꼈습니다.  
  ...
  nextjs: {
    ...
    Next.js는 단순한 프레임워크가 아닌, 프론트엔드와 백엔드의 경계를 넘나드는 풀스택 개발 도구라고 느껴졌습니다.  
    지금도 공식 문서를 기반으로 App Router의 최신 기능들을 꾸준히 학습하고 있으며, 앞으로의 프로젝트에서도 적극적으로 활용하고자 합니다.
    
    Next.js를 통해 저는 단순히 기술을 배우는 수준을 넘어서, 서비스 전체의 흐름을 설계하고 관리할 수 있는 개발자로 성장할 수 있다는 자신감을 얻었습니다.
    `,
  },
  nodejs: {
    ...
    아직 완전히 마스터한 상태는 아니지만, 직접 백엔드 API를 만들어보고 프론트엔드에서 호출해보는 과정을 반복하면서 점차 프론트-백엔드 간의 연결 구조를 체득해가고 있습니다.  
    ...
    앞으로도 백엔드에 대한 학습을 이어가며, 프론트엔드와 백엔드를 아우를 수 있는 유연한 개발자로 성장하고자 합니다.
    `,
  },
  certificate1: {
    ...
    단순한 암기 위주의 학습이 아닌, 이해를 바탕으로 한 사고력 중심의 공부를 통해 실제 개발에 적용 가능한 기본기를 쌓아가고자 합니다.
    
    정보처리기사 자격증 준비는 단순한 취득을 넘어서, 기초 CS 지식에 대한 자신감을 높이고, 개발자로서의 기반을 다지는 계기가 되어주고 있습니다.
    `,
  },
  certificate2: {
    label: "🛠️ 2025 SQLD",
    ...
    앞으로도 지속적으로 데이터 관련 역량을 강화하며, 프론트엔드와 백엔드를 아우르는 시각으로 완성도 높은 애플리케이션을 만들 수 있는 개발자로 성장하고자 합니다.
    `,
  },
};
